    ...
    const promises = points.map(async (point) => {
      const [waste] = await connection.execute(
        `
              ...
              WHERE collection_point_id = ? AND waste_status.status = 'ACTIVE'`,
        [point.cnpj]
      );
...
const getCollectionPoint = async ({ login, password }) => {
  const [point] = await connection.execute(
    `SELECT * FROM collection_points WHERE email='${login}' AND password='${password}'`
  );
  return point;
};
...
const getCollectionPointByWastes = async ({ wastes }) => {
  ...
  if (wastesToFilter.length >= 1) {
    const collectionPointIds = wastesToFilter.map(
      (waste) => waste.collection_point_id
    );
...
const getCollectionPointByCnpj = async (cnpj) => {
  const [point] = await connection.execute(` SELECT
    ...
    LEFT JOIN waste_status ON collection_points.cnpj = waste_status.collection_point_id  AND waste_status.status = 'ACTIVE'
